...
export function HappyBirthdaySlide({ isActive }: HappyBirthdaySlideProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const attractPct = 0.15;
      const attractStrength = 50;
      ...
      for (const p of particles) {
        ...
        if (useWand) {
          ...
          const attractDist = Math.max(ib.w, ib.h) * attractPct;
          if (mouseDist < attractDist && mouseDist > 1) {
            const invStr = Math.max(300 - attractStrength, 10);
      ...
      // Sparkle cluster around wand pointer (only during face phase)
      if (useWand) {
        for (let i = 0; i < 6; i++) {
